...
%%
for n=1:10
    [A1,ls]=MAT_A(A,prom,x,l,L,X1);
    x2(Data)=[];
    b=length(x2);
    A1(:,Data)=[];
    A1(:,Data+b)=[];
    A1=[A1(:,1:2*b)*(-1),A1(:,2*b+1:end)];
    x(Data)=[];
    x(Data+b)=[];
    if n>2
        if v-ls<q
    ...
    dx=-(A1'*P*A1)^(-1)*A1'*P*ls;
    x=x+dx;
    v=A1*dx+ls;
     X3=[x2,x(1:b),x((b+1):(b*2));Data2];
     X1=[sortrows(X3,1),x(b*2+1:end)];
     x=[X1(:,2);X1(:,3);X1(:,4)];
     x2=[X1(:,1)];
end
s0=sqrt((v'*P*v)/(size(A1,1)-size(A1,2)));
Mx=1^2*(A1'*P*A1)^(-1);
SMX=sqrt(diag(Mx));
